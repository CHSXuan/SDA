...
function validDate(value){return typeof value==='string'&&Number.isFinite(Date.parse(value))?new Date(value).toISOString():null;}
function info(store,id){const root=rootFor(store,id),manifest=JSON.parse(fs.readFileSync(path.join(root,'hrtf-set.json'),'utf8'));let metadata={};try{metadata=JSON.parse(fs.readFileSync(path.join(root,'profile-info.json'),'utf8'))}catch{}
 let assessment=null;try{assessment=JSON.parse(fs.readFileSync(path.join(root,'assessment.json'),'utf8'))}catch{}
 const recorded=validDate(metadata.createdAt)||validDate(assessment?.createdAt)||validDate(manifest.createdAt);
 const createdAt=recorded||fs.statSync(root).birthtime.toISOString();
 return {createdAt,createdAtSource:metadata.createdAtSource||(recorded?'recorded':'file'),id,name:metadata.name||manifest.source?.name||'个人 HRTF',directions:manifest.positions.length,method:manifest.source?.method||'imported-sofa'};}
function rename(store,id,name){const root=rootFor(store,id);const existing=info(store,id);const target=path.join(root,'profile-info.json'),temp=target+'.'+crypto.randomUUID()+'.tmp';
 fs.writeFileSync(temp,JSON.stringify({name:cleanName(name),createdAt:existing.createdAt,createdAtSource:existing.createdAtSource},null,2));fs.renameSync(temp,target);return info(store,id);}
function readPayload(store,id){const root=rootFor(store,id),manifest=JSON.parse(fs.readFileSync(path.join(root,'hrtf-set.json'),'utf8'));
 ...
 return {format:'sda-phrtf',version:1,name:info(store,id).name,createdAt:info(store,id).createdAt,createdAtSource:info(store,id).createdAtSource,manifest,assessment,files};}
...
function install(store,payload){const files=validate(payload),id=payload.manifest.subjectId,target=rootFor(store,id);fs.mkdirSync(store,{recursive:true});
 ...
 fs.writeFileSync(path.join(temp,'profile-info.json'),JSON.stringify({name:payload.name,createdAt:validDate(payload.createdAt)||validDate(payload.assessment?.createdAt)||new Date().toISOString(),createdAtSource:payload.createdAtSource==='file'?'file':'recorded'},null,2));fs.renameSync(temp,target);
...
function copy(store,id,name){const payload=readPayload(store,id);payload.name=cleanName(name);payload.createdAt=new Date().toISOString();payload.createdAtSource='recorded';payload.manifest.subjectId='personal-'+hash(crypto.randomUUID());return install(store,payload);}
