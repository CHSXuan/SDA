import {PersonalHrtfAudition,speakerTrials,measuredLayoutMotionTrials,trialStart,trialEnd,testVisualPosition,answerError} from "./phrtf-core.mjs";
...
export function openPhrtfTest(state,request) {
  ...
  const initialLayout=JSON.stringify(positions),previousHead=state.head,reference=/^(?:ku100|h(?:[3-9]|1[0-9]|20))$/.test(state.head)?state.head:"ku100";
  const dialog=el("dialog"),heading=el("h2","实测耳廓 · 播放验证"),status=el("p"),question=el("p"),controls=el("div"),canvas=el("canvas");
  dialog.className="phrtf-web-test";canvas.width=480;canvas.height=270;canvas.setAttribute("aria-label","测试方向示意图");
  let trials=speakerTrials(positions,reference),index=0,phase="screen",confirmations=[],answers=[],busy=false,heard=false,closed=false,epoch=0,animation=0,total=0,canSave=false;
  ...
  controls.append(replay,yes,no,save,exit);dialog.append(heading,el("p","测试音由手机直接播放，不经过主机的房间或监听。保存时会使用当前内置实测 HRTF 的完整响应；否只重试听。"),confirmLabel,gainLabel,canvas,status,question,controls);document.body.append(dialog);
  ...
  function answer(response) {
    ...
    if(!response){void listen();return;}
    if(phase==="screen")confirmations.push(record);
    index++;
    if(index>=trials.length){
      if(phase==="screen"){trials=measuredLayoutMotionTrials(positions,reference);phase="validation";index=0;}
  ...
  async function saveResult(){
    ...
    try{await request("hrtfGenerate",{assessment:{version:7,method:"measured-proxy-validation",subject:"generated",referenceSubject:reference,confirmations,answers,
      renderMethod:"speaker-vbap-v1",createdAt:new Date().toISOString(),dataset:"SADIE II",output:"remote-browser",previousHead,gainDb:Number(gain.value),responsesTotal:total,historyTruncated:total>64}});
