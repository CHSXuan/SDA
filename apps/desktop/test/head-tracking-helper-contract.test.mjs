...
assert.match(main, /if \(typeof helperPath !== "string" \|\| fs\.lstatSync\(helperPath\)\.isSymbolicLink\(\) \|\| !fs\.statSync\(helperPath\)\.isFile\(\)\) return false/);
assert.match(main, /spawn\(helperPath, \[\], \{ stdio: \["pipe", "pipe", "pipe"\], windowsHide: true \}\)/);
assert.match(main, /process\.resourcesPath, "head-tracking-helper", `SdaAirPodsHeadTracking\$\{ext\}`/);
assert.match(main, /source: "bundled-helper"/);
assert.match(main, /!\["windows-airpods-experimental", "macos-airpods"\]\.includes\(message\.source\)/);
