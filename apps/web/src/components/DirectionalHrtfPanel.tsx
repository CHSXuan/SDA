...
export default function DirectionalHrtfPanel({codec}:{codec?:string}={}){
  ...
  useEffect(()=>{
    ...
    const check=async()=>{try{
      ...
      if(alive)setStatus(!output?.running?"已保存 · 等待播放":is360Ra?"360RA 13 · 按要求使用旧路径":output.hrtfReady===false?"已保存 · 等待 HRTF 就绪":cinema?.settings.monitor?.hardware?.enabled?"已启用 · 独立对象硬件链 + 方向 HRTF":"已启用 · 对象方向直达声 + 独立房间反射");
