...
export default function RoomLab({layout,snapshot,onRecall,onCompare,onRestore,onVisual,comparison,visualSpeaker,audition,onAudition,onCalibration,onApply,onDisable,onClose}: {
  ...
  return <section className="panel float-panel room-lab desktop-sheet" aria-label="房间实验室">
    ...
    {view==="room"&&<>
      ...
      <div className="room-lab-fields">{numeric("length","前后长度 m",3,10,.1)}{numeric("width","左右宽度 m",3,8,.1)}{numeric("height","房间高度 m",2.2,4,.1)}{numeric("earHeight","耳部高度 m",.8,1.6,.1)}{(config.material!=="studio"||config.shape==="box")&&numeric("placement","音箱径向比例",.5,1,.05)}{numeric("order","反射阶数",1,12,1)}</div>
      ...
      {config.material==="studio"&&config.shape!=="box"&&<label className="cinema-number"><span>等距监听距离</span><input type="number" aria-label="等距监听距离" min={.8} max={2.5} step={.1} value={config.listeningDistance??1.2} disabled={busy||(room?.builtin&&!custom)} onChange={e=>{const d=e.currentTarget.valueAsNumber;if(Number.isFinite(d))setConfig(v=>({...v,listeningDistance:Math.max(.8,Math.min(2.5,d))}));}}/><small>m</small></label>}
      {simulation?.studioDesign&&<p className="cinema-source">{simulation.config.shape==="box"?"方形音箱布置":`等距音箱阵列 ${simulation.studioDesign.nearFieldDistanceMetres} m`} · 偏干控制室 · 中频混响估算 {Math.min(...simulation.studioDesign.eyringSeconds.slice(1,6)).toFixed(2)}–{Math.max(...simulation.studioDesign.eyringSeconds.slice(1,6)).toFixed(2)} s（非实测）</p>}
    ...
    {view==="paths"&&<>
      ...
      <label className="room-animation-speed"><span>慢放 {slowdown} 倍</span><Slider  aria-label="传播慢放倍率" min="25" max="200" step="25" value={slowdown} disabled={!paths} onChange={e=>setSlowdown(Number(e.target.value))}/></label>
