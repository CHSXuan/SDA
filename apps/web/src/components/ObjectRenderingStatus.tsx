...
export default function ObjectRenderingStatus({direct}:{direct:boolean}) {
  ...
  useEffect(()=>{
    ...
    const update=async()=>{
      ...
      try {
        ...
        const status=!output?.running?`已保存：${mode}，等待播放`:!output.hrtfReady?`已保存：${mode}，等待 HRTF 就绪`:cinema?.settings.monitor?.hardware?.enabled?"硬件仿真开启，当前使用虚拟扬声器处理":`当前：${mode}`;
  ...
  return <div className="settings-description" role="status"><p>{detail}</p><p>基础逐对象、连续方向与近场可同时开启，共用对象渲染通路。连续方向或近场开启时，会自动使用逐对象处理；三个都关闭才回到虚拟扬声器路径。</p></div>;
}
