...
export default function NearFieldPanel(){
  ...
  return <fieldset className="settings-group settings-section" disabled={busy}>
    ...
    <label className="source-extent-control"><span>坐标单位对应 <output>{scale.toFixed(2)} m</output></span><Slider aria-label="近场距离映射"  min="0.25" max="4" step="0.05" value={scale} onChange={e=>setScale(Number(e.target.value))}/></label>
    <p className="settings-description">对象坐标不是实际米数。映射后距中心 0.2–1 m 的对象受到处理；更近按 0.2 m 计算，更远保持原样。距离映射不是母版测量值。</p>
    <div className="cinema-actions"><button data-button="primary" onClick={()=>void apply({enabled:true,metresPerUnit:scale})}>应用距离</button><button onClick={()=>void apply({enabled:false,metresPerUnit:1})}>恢复默认</button></div>
