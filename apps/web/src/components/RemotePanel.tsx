...
export default function RemotePanel({status}:{status:RemoteStatus}) {
  ...
  return <section className="remote-panel" aria-label="多设备无损远程">
    ...
    {status.role!=="client"&&<section className="remote-device-list remote-tab-content" role="tabpanel" id="remote-page-devices" aria-labelledby="remote-tab-devices" hidden={tab!=="devices"} aria-label="设备授权">
      ...
      {(status.pendingDevices??[]).map(device=><div className="remote-device" key={device.id}><strong>{device.name}</strong><small>{device.address} · 请求连接</small><div><button disabled={busy} onClick={()=>void run("deviceApprove",{id:device.id,canControl:true})}>允许收听与控制</button><button disabled={busy} onClick={()=>void run("deviceApprove",{id:device.id,canControl:false})}>仅收听</button><button disabled={busy} onClick={()=>void run("deviceApprove",{id:device.id,canControl:true,canListen:false})}>仅控制</button><button disabled={busy} onClick={()=>void run("deviceReject",{id:device.id})}>拒绝</button></div></div>)}
      {(status.devices??[]).map(device=>{const peer=status.connectedDevices?.find(p=>p.id===device.id);const online=!!peer;return <div className="remote-device" key={device.id}><div className="remote-device-name"><Smartphone size={18}/><strong>{device.name}</strong><span className={online?"remote-online":"remote-offline"}>{online?(peer?.controlOnly?"仅控制":"收听中"):"离线"}</span></div><div><Select aria-label={`${device.name} 权限`} value={device.canListen===false?"control-only":device.canControl?"control":"listen"} disabled={busy} onChange={e=>void run("devicePermission",{id:device.id,canControl:e.target.value!=="listen",canListen:e.target.value!=="control-only"})}><option value="control">收听与控制</option><option value="listen">仅收听</option><option value="control-only">仅控制</option></Select>{online&&<button disabled={busy} onClick={()=>void run("deviceDisconnect",{id:device.id})}>断开</button>}<button disabled={busy} onClick={()=>void run("deviceRevoke",{id:device.id})}>撤销授权</button></div></div>;})}
...
export function RemoteClientView({status}:{status:RemoteStatus}) {
  ...
  return <div className="app remote-client"><WindowTitlebar/><div className="remote-client-content">
    ...
    <footer className="remote-client-footer"><div><strong>{status.output?.actualName??"正在打开收听设备"}</strong><small>{status.output?.mode==="directsound"?"DirectSound":status.output?.mode==="asio"?"ASIO":status.output?.mode==="exclusive"?`${isMac?"CoreAudio":"WASAPI"} 独占`:`${isMac?"CoreAudio":"WASAPI"} 共享`} · {status.output?.sampleFormat??"48 kHz"} · 缓冲 {Math.round(status.queuedMs)} ms · 已接收 {(status.bytes/1048576).toFixed(1)} MB</small></div><button onClick={()=>void window.sdaDesktop?.remoteSession?.("stop").catch(e=>setError(String(e)))}><Unplug size={16}/><span>断开连接</span></button></footer>
