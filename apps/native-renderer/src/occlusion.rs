...
pub fn ear_openness(
    ...
    // Head-relative side: the ear away from the source azimuth is blocked,
    // the ear on the source side stays open (direct wave grazes past).
    let relative = crate::spatial::head_relative_adm(position, head);
    let side = relative[0].signum(); // +x = left in ADM
    let far = if side >= 0.0 { 1 } else { 0 };
...
pub fn ear_openness_prepared(
    ...
    let side = position_relative[0].signum();
    let far = if side >= 0.0 { 1 } else { 0 };
    let mut openness = [1.0_f32; 2];
    openness[far] = 1.0 - 0.4 * occluded;
    openness
...
mod tests {
    ...
    fn near_same_bearing_object_shadows_far_ear() {
        // Occluder 4x nearer on the same bearing: the far ear must close.
        ...
        assert!((openness[0] - openness[1]).abs() > 0.2, "shadow must be asymmetric: {openness:?}");
